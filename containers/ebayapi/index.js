...
async function getGPU(gpu) {
  ...
  const url = [
    ...
    'OPERATION-NAME=findItemsAdvanced&', 'SERVICE-VERSION=1.13.0&',
    ...
    'itemFilter(1).name=ListingType&', 'itemFilter(1).value(0)=FixedPrice&',
    'itemFilter(1).value(1)=AuctionWithBIN&',
    'itemFilter(1).value(2)=StoreInventory&',
    'itemFilter(2).name=LocalPickupOnly&', 'itemFilter(2).value=false&',
